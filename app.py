...
def forecast():
    ...
    if request.method == "GET":
        ...
        if forecast_type == "monthly":
            ...
            historicalDataPath = os.environ.get("HISTORICAL_DATA_PATH_MONTHLY")
            ...
            historicalDataPath = os.environ.get("HISTORICAL_DATA_PATH_WEEKLY")
        try:
...
def accuracy():
    if request.method == "GET":
        ...
        try:
            ...
            return render_template(
                "accuracy.html", status="False", message="Invalid URL"
            )
        if (
            (DATA_WITHOUT_OUTLIER_PATH is None)
            or (forecast_length is None)
        ...
        if True:
            ...
            if type(data_df) is dict:
                return render_template(
                    "accuracy.html", status="False", message=data_df["message"]
                )
            mse, mape, bias = utils.calculate_forecast_accuracy(
                data_df,
                forecast_length,
                date_col,
                value_col,
            ...
            return render_template(
                "accuracy.html", status="True", mse=mse, mape=mape, bias=bias
            )
        else:
            return render_template(
                "accuracy.html",
                status="False",
                message="The algorithm is facing an issue to forecast the data.",
            )
